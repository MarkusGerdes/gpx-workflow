...
# === SCRIPT METADATA ===
SCRIPT_NAME = "3_analyze_peaks_plot.py"
SCRIPT_VERSION = "3.0.0"
SCRIPT_DESCRIPTION = "Peak analysis and elevation profiling with place annotations, algorithm tracking and standardized metadata"
...
# === CHANGELOG ===
CHANGELOG = """
v1.0.0 (pre-2025): Comprehensive peak analysis with surface information overlay
v2.0.0 (2025-06-07): Implemented full standardized metadata system with processing history
v3.0.0 (2025-06-07): Enhanced with algorithm parameter tracking and performance optimization
"""
...
def main(args, config: Config):
    ...
    print("Starte Peak/Profil Analyse V8 (mit dynamischem Orts-Offset und Metadaten)...")
    MAX_PEAKS_TO_ANALYZE = 4 # NEU: Definiere, wie viele Peaks maximal analysiert werden sollen
    
    # Tracking-Variablen für Algorithmus-Performance
    algorithm_metadata = {
        "algorithm_version": "V8_dynamic_offset",
        "peak_detection_algorithm": "scipy.signal.find_peaks",
        "smoothing_algorithm": "scipy.signal.savgol_filter",
        "max_peaks_analyzed": MAX_PEAKS_TO_ANALYZE,
        "peak_prominence_threshold_m": None,
        "gain_threshold_m": None,
        "total_peaks_found": 0,
        "peaks_analyzed": 0,
    ...
    # Speichere wichtige Parameter für Metadaten
    algorithm_metadata["peak_prominence_threshold_m"] = config.min_peak_prominence_m
    algorithm_metadata["gain_threshold_m"] = config.gain_threshold
    algorithm_metadata["processing_stages"].append(f"Config loaded at {datetime.now().isoformat()}")
    ...
    try: track_df = pd.read_csv(input_csv_path, parse_dates=['Time'], encoding='utf-8', comment='#')
    ...
    if args.places_coords_csv and os.path.exists(args.places_coords_csv):
         try:
             places_coords_df = pd.read_csv(args.places_coords_csv, comment='#')
    ...
    if args.relevant_pois_csv and os.path.exists(args.relevant_pois_csv): # NEUES Argument
        try:
            all_relevant_pois_df = pd.read_csv(args.relevant_pois_csv, comment='#')
    ...
    if args.surface_data_csv and os.path.exists(args.surface_data_csv):
        try:
            surface_data_df = pd.read_csv(args.surface_data_csv, comment='#')
    ...
    # --- 3. Finde Peaks ---
    algorithm_metadata["processing_stages"].append(f"Peak detection started at {datetime.now().isoformat()}")
    peak_indices, properties = find_peaks(elev_smooth, prominence=config.min_peak_prominence_m)
    algorithm_metadata["total_peaks_found"] = len(peak_indices)
    ...
    if len(peak_indices) > 0:
        ...
        algorithm_metadata["peaks_analyzed"] = len(peaks_to_analyze_indices)
    ...
    # Zähle valide Segmente für Metadaten
    algorithm_metadata["valid_segments_found"] = len([s for s in all_segments_combined if s.gain_m >= config.gain_threshold])
    algorithm_metadata["processing_stages"].append(f"Peak analysis completed at {datetime.now().isoformat()}")
    ...
    # Speichere mit vollständigen Metadaten
    processing_parameters = {
        'smooth_window': config.smooth_window,
        'smooth_poly': config.smooth_poly,
        'gain_threshold_m': config.gain_threshold,
        'peak_prominence_m': config.min_peak_prominence_m,
        'max_peaks_analyzed': MAX_PEAKS_TO_ANALYZE,
        'pause_min_duration_s': config.pause_min_duration_s,
        'pause_max_distance_m': config.pause_max_distance_m
    }
    
    additional_metadata = {
        'algorithm_version': algorithm_metadata['algorithm_version'],
        'processing_duration_seconds': (datetime.now() - run_start_time).total_seconds(),
        'total_track_points': n_points,
        'track_distance_km': track_len_km,
        'elevation_range_m': f"{elev_smooth.min():.1f} - {elev_smooth.max():.1f}",
        'total_peaks_found': algorithm_metadata['total_peaks_found'],
        'peaks_analyzed': algorithm_metadata['peaks_analyzed'],
        'valid_segments_found': algorithm_metadata['valid_segments_found'],
        'data_quality': 'high' if len(peaks_to_analyze_indices) > 0 else 'low'
    }
    
    try: 
        output_dir = os.path.dirname(args.output_peak_csv); os.makedirs(output_dir, exist_ok=True)
        
        # CSV mit vollständigen Metadaten schreiben
        write_csv_with_metadata(
            dataframe=peak_segment_df,
            output_path=args.output_peak_csv,
            script_name=SCRIPT_NAME,
            script_version=SCRIPT_VERSION,
            input_files=[input_csv_path] + ([args.places_coords_csv] if args.places_coords_csv else []) + ([args.surface_data_csv] if args.surface_data_csv else []),
            processing_parameters=processing_parameters,
            api_metadata=None,  # Kein API verwendet
            additional_metadata=additional_metadata,
            float_format='%.3f'
        )
        
        print(f"  -> Peak/Segment Daten mit Metadaten gespeichert: {args.output_peak_csv}")
        
    except Exception as e: 
        print(f" Fehler beim Speichern der Peak/Segment Daten mit Metadaten '{args.output_peak_csv}': {e}")
        # Fallback
        peak_segment_df.to_csv(args.output_peak_csv, index=False, encoding='utf-8', float_format='%.3f')
        print(f"  -> Fallback Peak/Segment Daten gespeichert: {args.output_peak_csv}")
