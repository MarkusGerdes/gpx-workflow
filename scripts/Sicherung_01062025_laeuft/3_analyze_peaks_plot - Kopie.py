...
def plot_profile(base_filename: str,
    ...
    peak_colors = config.peak_highlight_colors # Neue Methode mit Liste aus Config
    peak_handles = []
    peak_labels_text = []
    for i, current_peak_idx in enumerate(peak_indices): # Die Schleifenvariable ist current_peak_idx
        if i >= len(peak_colors): 
            color = "#7f7f7f" 
        else:
            color = peak_colors[i]
        
        # Definiere peak_x_km und peak_y_m FÜR DEN AKTUELLEN PEAK IN DER SCHLEIFE
        peak_x_km = dist_km[current_peak_idx]  # Verwende current_peak_idx
        peak_y_m = elev_m[current_peak_idx]    # Verwende current_peak_idx
        ...
        # Verwende die gerade definierten peak_x_km und peak_y_m
        ...
        ax.text(peak_x_km + 0.15 * (ax.get_xlim()[1] - ax.get_xlim()[0])/100 , # Kleiner horizontaler Offset für den Text
